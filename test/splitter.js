...
contract('Splitter', (accounts) => {
    ...
    it('should store Alice, Bob & Carol init data', async () => {
        ...
        const expectAliceAddress = accounts[0];

        const tempSplitterInstance = await Splitter.new(expectedBobAddress, expectedCarolAddress);

        const alice = await tempSplitterInstance.alice.call();
        const bob = await tempSplitterInstance.bob.call();
        const carol = await tempSplitterInstance.carol.call();

        assert.equal(alice, expectAliceAddress, "Alice address was not stored correctly");

        assert.equal(bob[0], expectedBobAddress, "Bob address was not stored correctly");
        assert.equal(bob[1], 0, "Bob balance was not initialized correctly");

        assert.equal(carol[0], expectedCarolAddress, "Carol address was not stored correctly");
